...
import { Wallet, TrendingUp, Target, Award, Gift, Sparkles, Info, ChevronRight, ChevronLeft, MessageSquare, GripVertical, Package, ArrowRightLeft, Lock, Swords } from 'lucide-react';
...
const KidDashboard: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl mx-auto px-4 pb-20">
      ...
      <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 sticky top-4 z-40 backdrop-blur-md gap-0.5">
        {([
          ['dashboard', Wallet, '儀表板'],
          ['badges', Award, '成就牆'],
          ['treasure', Gift, '寶物庫'],
          ['adventure', Swords, '冒險區'],
        ] as const).map(([t, Icon, label]) => (
          <button key={t} onClick={() => setActiveTab(t)}
            className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-xs font-bold transition-all ${activeTab === t ? 'bg-primary text-white shadow-lg' : 'text-muted hover:text-white'}`}>
            <Icon size={16} /> {label}
          </button>
        ))}
      </div>
