...
import {
  ...
  ChevronUp, Copy, GripVertical, ArrowRightLeft, Save, Lock,
} from 'lucide-react';
...
type SectionId =
  | 'pending' | 'chart' | 'categories' | 'quests'
  | 'kids' | 'allowance' | 'allowance-schedule' | 'control' | 'history';
...
const SortableSection: React.FC<SortableSectionProps> = ({ id, title, emoji, children, defaultOpen = true }) => {
    ...
    <div ref={setNodeRef} style={style} className={CARD}>
      ...
      <div className="flex items-center gap-3 mb-0">
        ...
        <button
          {...attributes}
          {...listeners}
          className="p-3 -ml-2 text-slate-500 hover:text-slate-300 cursor-grab active:cursor-grabbing touch-none"
          aria-label="拖曳排序"
        >
...
const ParentDashboard: React.FC = () => {
  ...
  const SECTION_DEFAULTS: SectionId[] = ['pending', 'kids', 'quests', 'chart', 'categories', 'allowance', 'allowance-schedule', 'control', 'history'];
  const [sectionOrder, setSectionOrder] = useState<SectionId[]>(() => {
    try {
      const saved = localStorage.getItem('pd-section-order');
      return saved ? JSON.parse(saved) : SECTION_DEFAULTS;
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      localStorage.setItem('pd-section-order', JSON.stringify(next));
    }
  };
  ...
  // Chart data — useMemo must be before any early return
  ...
  const onImportFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        setImportStatus({ type: 'error', message: (err as Error)?.message || '匯入失敗' });
      }
    };
  ...
  const submitQuest = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!qKid || !qTitle) return;
    let dueDate: string | undefined;
    if (qDueMode === 'days') {
      dueDate = new Date(Date.now() + Number(qDueDays) * 86400000).toISOString().split('T')[0];
    } else {
      dueDate = qDueDate;
    }
    const boxId = qBoxRewardId || undefined;
    if (qKid === 'all') {
      await Promise.all(activeKids.map(k => createQuest(k.id, qTitle, +qExp, +qMoney, +qGem, qIcon, dueDate, boxId)));
    } else {
      await createQuest(qKid, qTitle, +qExp, +qMoney, +qGem, qIcon, dueDate, boxId);
    }
  ...
  const renderSection = (id: SectionId) => {
    switch (id) {
      ...
      case 'pending':
        ...
        return (
          <SortableSection key={id} id={id} emoji="🔔" title={`待審事項 (${pendingCount})`} defaultOpen={true}>
              ...
              {pendingClaims.map(c => {
                ...
                return (
                  <div key={c.id} className="rounded-2xl border border-pink-500/20 bg-pink-500/5 p-4 flex items-center gap-3">
                    ...
                    <button onClick={() => approveTreasureClaim(c.id)} className="flex-shrink-0 px-3 py-2 rounded-xl text-xs font-bold transition-colors bg-pink-500/20 text-pink-400 hover:bg-pink-500/30">
                      核准發放
                    </button>
                  </div>
                );
                ...
                return (
                  <div key={t.id} className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-4 flex items-center gap-3">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => approveTransaction(t.id)} className="px-3 py-2 rounded-xl text-xs font-bold bg-indigo-500/20 text-indigo-300 hover:bg-indigo-500/30 transition-colors">
                        核准
                      </button>
                      <button onClick={() => { if(confirm('確定駁回此支出並退還金額？')) rejectTransaction(t.id); }} className="px-3 py-2 rounded-xl text-xs font-bold bg-red-500/10 text-red-500/60 hover:text-red-400 transition-colors">
                        駁回
                      </button>
                    </div>
                  </div>
      ...
      case 'kids':
        return (
          <SortableSection key={id} id={id} emoji="👨‍👩‍👧" title="孩子帳戶" defaultOpen={true}>
                ...
                return (
                  <div key={kid.id} className={`rounded-2xl border ${kid.isHidden ? 'border-white/5 opacity-60' : 'border-white/10'} overflow-hidden`}>
                    ...
                    <div className="flex items-center gap-3 p-4 bg-white/5">
                      <div className="w-11 h-11 rounded-full bg-gradient-to-br from-primary/50 to-info/50 flex items-center justify-center font-black text-lg flex-shrink-0">
                        {kid.name.charAt(0)}
                      </div>
                      {editingKidId === kid.id ? (
                        ...
                        <>
                          <div className="flex-1 min-w-0">
                            ...
                            {/* Google account binding indicator */}
                            {(kid.googleUid || kid.googleEmail) ? (
                              <div className="flex items-center gap-1 mt-1">
                                <span className="text-[10px] bg-blue-500/15 border border-blue-500/25 text-blue-300 px-2 py-0.5 rounded-full flex items-center gap-1">
                                  <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" className="opacity-80"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
                    ...
                    {/* Expand / collapse */}
                    <button
                      onClick={() => setExpandedKidId(isOpen ? null : kid.id)}
                    ...
                    {isOpen && (
                      <div className="p-4 border-t border-white/5 bg-black/20 flex flex-col gap-5">
                          ...
                          {pinEditingId === kid.id ? (
                            <div className="flex gap-2">
                              <input
                                ...
                                value={pinEditingValue}
                                onChange={e => setPinEditingValue(e.target.value.replace(/\D/g, '').slice(0, 4))}
                                maxLength={4}
                                placeholder="4 位數字"
                                autoFocus
                              />
                              <button
                                onClick={() => { if (pinEditingValue.length === 4) { setUserPin(kid.id, pinEditingValue); setPinEditingId(null); } }}
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-lg font-bold tracking-[0.3em] text-white bg-black/40 px-3 py-1 rounded-lg border border-white/5">
                                  {kid.pin || '0000'}
                                </span>
                                <span className="text-[10px] text-slate-500">家長可直接查看</span>
                              </div>
                              <button 
                                onClick={() => { setPinEditingId(kid.id); setPinEditingValue(kid.pin || '0000'); }}
                                className="text-xs text-primary hover:text-primary-focus font-bold flex items-center gap-1"
                              >
                                <Edit2 size={12} /> 修改
                              </button>
                            </div>
                        ...
                        {/* Individual Allowance Settings */}
                        <div className="flex flex-col gap-3 p-3 bg-indigo-500/5 rounded-2xl border border-indigo-500/10">
                          <div className="flex items-center justify-between">
                            <div className="text-xs text-indigo-300 font-bold uppercase flex items-center gap-1.5">
                              <TrendingUp size={12} /> 自動派發設定
                            </div>
                            {editingKidAllowId !== kid.id && (
                              <button 
                                onClick={() => startEditingKidAllowance(kid)}
                          ...
                          {editingKidAllowId === kid.id ? (
                            <div className="flex flex-col gap-3 animate-in fade-in duration-300">
                              ...
                              <div className="flex flex-col gap-1">
                                ...
                                {kaMode === 'weekly' ? (
                                  <div className="flex justify-between gap-1">
                                    {[1,2,3,4,5,6,0].map(d => (
                                      <button key={d} onClick={() => setKaDays(prev => prev.includes(d) ? prev.filter(x => x !== d) : [...prev, d])} className={`flex-1 py-1.5 rounded-lg text-[10px] font-bold ${kaDays.includes(d) ? 'bg-indigo-500 text-white' : 'bg-white/5 text-muted'}`}>
                                        {['日','一','二','三','四','五','六'][d]}
                                      </button>
                                    ))}
                                  </div>
                              ...
                              <div className="flex gap-2 pt-1">
                                <button 
                                  onClick={() => {
                                    if (kaDays.length === 0) return alert('請選擇派發日期');
                                    updateKidAllowance(kid.id, Number(kaAmount), Number(kaRatio), { period: kaMode, days: kaDays, hour: kaHour, minute: kaMinute });
                                    setEditingKidAllowId(null);
                                  }}
                                  className={`${BTN} flex-1 bg-indigo-500 text-white text-xs h-10`}
                                >
                                  儲存設定
                                </button>
                                <button 
                                  onClick={() => setEditingKidAllowId(null)}
                                  className={`${BTN} px-3 bg-white/10 text-muted text-xs h-10`}
                                >
                                  取消
                                </button>
                              </div>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          {kidEnvs.map(env => {
                            ...
                            return (
                              <KidEnvelopeRow
                                ...
                                toggleHideEnvelope={toggleHideEnvelope}
                          ...
                          {expandedEnvKidId === kid.id ? (
                            ...
                            <button onClick={() => setExpandedEnvKidId(kid.id)} className={`${BTN} w-full bg-white/5 text-muted hover:bg-white/10 hover:text-white`}>
                              <PlusCircle size={16} /> 新增信封
                            </button>
                          )}
                        </div>
                      </div>
      ...
      case 'quests':
        return (
          <SortableSection key={id} id={id} emoji="⚔️" title="任務管理" defaultOpen={true}>
            {/* Tab switcher */}
            ...
            {questTab === 'single' ? (
              <form onSubmit={submitQuest} className="flex flex-col gap-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { label: '⭐ EXP', val: qExp, set: setQExp },
                    { label: '💰 金錢', val: qMoney, set: setQMoney },
                    { label: '💎 寶石', val: qGem, set: setQGem },
                  ].map(({ label, val, set }) => (
                    <div key={label}>
      ...
      case 'chart':
        return (
          <SortableSection key={id} id={id} emoji="📊" title="花費統計" defaultOpen={false}>
            <div className="flex flex-col items-center gap-6">
              {/* Pie */}
              <div
                className="w-44 h-44 rounded-full border-4 border-white/5 shadow-xl"
                style={{
                  background: totalCat === 0
                    ? '#1e293b'
                    : `conic-gradient(${(() => {
                        let c = 0;
                        return categoryData.map((d, i) => {
                          const s = c, e = c + (d.value / totalCat) * 100;
                          c = e;
                          return `${COLORS[i % COLORS.length]} ${s}% ${e}%`;
                        }).join(', ');
                      })()})`
                }}
              />
      ...
      case 'categories':
        return (
          <SortableSection key={id} id={id} emoji="🏷️" title="花費類別" defaultOpen={false}>
            <div className="flex flex-col gap-4">
              {/* Add new category */}
              <form
                onSubmit={e => { e.preventDefault(); if (catName) { addExpenseCategory(catName, catIcon); setCatName(''); setCatIcon('🛒'); } }}
                ...
                {expenseCategories.map(cat => (
                  editingCatId === cat.id ? (
                    ...
                    <div key={cat.id} className="flex flex-col items-center gap-1.5 bg-primary/10 border border-primary/30 rounded-2xl p-3">
                      ...
                      <input
                        className="w-full bg-slate-900/70 border border-white/10 rounded-lg px-2 py-1 text-xs text-white focus:outline-none focus:border-primary text-center"
                        value={editCatName}
                      ...
                      <div className="flex gap-1 mt-0.5">
                        <button
                          onClick={() => { updateExpenseCategory(cat.id, editCatName, editCatIcon); setEditingCatId(null); }}
                          className="p-1.5 rounded-lg bg-green-500/20 text-green-400 hover:bg-green-500/30 transition-colors"
                        ><Check size={13} /></button>
                        <button
                          onClick={() => setEditingCatId(null)}
                          className="p-1.5 rounded-lg bg-white/10 text-muted hover:bg-white/20 transition-colors"
                        ><X size={13} /></button>
                      </div>
                    </div>
                  ) : (
                    ...
                    <div key={cat.id} className="relative group flex flex-col items-center gap-1.5 bg-white/5 border border-white/5 rounded-2xl p-3 hover:border-white/20 transition-colors">
                      ...
                      <div className="absolute top-1.5 right-1.5 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => { setEditingCatId(cat.id); setEditCatName(cat.name); setEditCatIcon(cat.icon); }}
                          className="p-1 rounded-lg bg-info/20 text-info hover:bg-info/30 transition-colors"
      ...
      case 'allowance': {
        ...
        return (
          <SortableSection key={id} id={id} emoji="💸" title="零用錢派發" defaultOpen={false}>
            <div className="flex flex-col gap-4">
                ...
                {(['auto', 'manual'] as const).map(tab => (
                  <button key={tab} onClick={() => setAllowTab(tab)}
                    className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all ${
                      allowTab === tab ? 'bg-primary text-white shadow-lg' : 'text-muted hover:text-white'
                    }`}>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div>
                  ...
                  <input type="range" min="0" max="100" step="5" className="w-full accent-primary"
                    value={walletPct} onChange={e => setAllowWalletPct(e.target.value)} />
              ...
              {allowTab === 'auto' && (
                <div className="flex flex-col gap-3 p-3 bg-black/20 rounded-2xl border border-white/5">
                  ...
                  <button
                    onClick={() => { if (confirm(`確定設定自動派發金額 ${currencySymbol}${totalAmt} 給${allowTarget === 'all' ? '所有孩子' : activeKids.find(k=>k.id===allowTarget)?.name}？`)) {
                    ...
                    className={`${BTN} w-full bg-primary/80 hover:bg-primary text-white`}
                  >💾 儲存派發金額設定</button>
                </div>
              )}

              {/* ── Manual tab: immediate dispatch ── */}
              {allowTab === 'manual' && (
                <div className="flex flex-col gap-3 p-3 bg-black/20 rounded-2xl border border-white/5">
                  <div className="text-xs text-muted font-bold uppercase">單次立即派發</div>
                  {/* Diagnostic: check if target kids have required envelopes */}
                  {(() => {
                    const targets = allowTarget === 'all' ? activeKids : activeKids.filter(k => k.id === allowTarget);
                  ...
                  <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 text-xs text-amber-300">
                    ⚠️ 將立即把 <strong>{currencySymbol}{totalAmt}</strong> 派發給{' '}
                    <strong>{allowTarget === 'all' ? '所有孩子' : activeKids.find(k=>k.id===allowTarget)?.name}</strong>，
                    並按 {walletPct}%/{piggyPct}% 分配到隨身錢包和存錢筒。
                  </div>
                  <button
                    onClick={() => {
                      if (totalAmt <= 0) { alert('請輸入有效金額'); return; }
                      if (confirm(`確定立即派發 ${currencySymbol}${totalAmt}？`)) {
                        manualDistributeAllowance(allowTarget, totalAmt, walletPct);
                      }
                    }}
                    className={`${BTN} w-full bg-gradient-to-r from-primary to-info text-white text-base font-black`}
                  >🚀 立即執行派發</button>
                </div>
              )}
      ...
      case 'allowance-schedule': {
        return (
          <SortableSection key={id} id={id} emoji="📅" title="自動派發排程" defaultOpen={false}>
            <div className="flex flex-col gap-4">
              <div className="text-xs text-muted leading-relaxed">
                設定系統自動發放零用錢的時間。系統會根據此設定自動計算下次派發日。
              </div>
              ...
              <div className="flex gap-1 bg-black/30 p-1 rounded-2xl border border-white/5">
                {(['weekly', 'monthly'] as const).map(mode => (
                  <button key={mode} onClick={() => setAllowMode(mode)}
                    className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all ${
                      allowMode === mode ? 'bg-primary text-white shadow-lg' : 'text-muted hover:text-white'
                    }`}>
              ...
              {allowMode === 'weekly' ? (
                  ...
                  <div className="grid grid-cols-7 gap-2">
                    {[{l:'日',v:0},{l:'一',v:1},{l:'二',v:2},{l:'三',v:3},{l:'四',v:4},{l:'五',v:5},{l:'六',v:6}].map(d => (
                      <button key={d.v} type="button"
                        onClick={() => setAutoAllowDays(p => p.includes(d.v) ? p.filter(x => x !== d.v) : [...p, d.v])}
                        className={`aspect-square rounded-full font-black text-sm transition-all active:scale-90 flex items-center justify-center ${
                          autoAllowDays.includes(d.v) ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'bg-white/10 text-muted hover:bg-white/20'
                        }`}>
                        {d.l}
                      </button>
                    ))}
                ...
                <div className="flex flex-col gap-3 p-4 bg-black/20 rounded-2xl border border-white/5">
                  ...
                  <div className="grid grid-cols-7 gap-1.5">
                    {Array.from({length: 31}, (_, i) => i + 1).map(d => (
                      <button key={d} type="button"
                        onClick={() => setMonthlyDay(String(d))}
                        className={`aspect-square rounded-lg font-bold text-xs transition-all active:scale-90 flex items-center justify-center ${
                          monthlyDay === String(d) ? 'bg-primary text-white shadow-lg' : 'bg-white/5 text-muted hover:bg-white/10'
                        }`}>
                        {d}
                      </button>
                    ))}
                  ...
                  <div className="text-[10px] text-slate-500 mt-1 italic">
                    * 若當月無此日期（如 30, 31 號），系統將於次月 1 號或當月最後一日處理（建議設定 1-28 號較為穩定）。
                  </div>
                </div>
              )}

              {/* Time Selection - Feature Request */}
              <div className="flex flex-col gap-3 p-4 bg-black/20 rounded-2xl border border-white/5">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <select
                      className={INPUT}
                      value={allowHour}
                      onChange={e => setAllowHour(Number(e.target.value))}
                    >
                      {Array.from({ length: 24 }).map((_, i) => (
                        <option key={i} value={i}>{String(i).padStart(2, '0')} 點</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="flex-1">
                    <select
                      className={INPUT}
                      value={allowMinute}
                      onChange={e => setAllowMinute(Number(e.target.value))}
                    >
                      {Array.from({ length: 12 }).map((_, i) => (
                        <option key={i * 5} value={i * 5}>{String(i * 5).padStart(2, '0')} 分</option>
                      ))}
                    </select>
                  </div>
                </div>
              ...
              <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-2xl p-4 flex flex-col gap-2">
                <div className="text-xs font-bold text-indigo-300 flex items-center gap-1.5">
                  <RefreshCw size={12} className="animate-spin-slow" /> 預計下次派發時程
                </div>
                    ...
                    return (
                      <div className="flex flex-col gap-1 mt-1">
                        ...
                        <span className="text-xs text-slate-400">
                          {allowMode === 'weekly' 
                            ? `每週 ${autoAllowDays.sort().sort((a,b)=>a-b).map(d => ['日','一','二','三','四','五','六'][d]).join('、')} ${String(allowHour).padStart(2, '0')}:${String(allowMinute).padStart(2, '0')} 自動執行`
              ...
              <button
                onClick={() => {
                  const days = allowMode === 'weekly' ? autoAllowDays : [Number(monthlyDay)];
                  if (days.length === 0) return alert('請至少選擇一個派發日期');
                  updateAllowanceSettings(allowMode, days, allowHour, allowMinute);
                  alert('自動派發設定已儲存！');
                }}
                className={`${BTN} w-full bg-primary text-white shadow-lg shadow-primary/20 mt-2`}
              >
                💾 儲存排程設定
              </button>
            </div>
      ...
      case 'control':
        return (
          <SortableSection key={id} id={id} emoji={familyIcon} title={familyName || '家長控制中心'} defaultOpen={false}>
            <div className="flex flex-col gap-6">

              {/* Family name + icon edit */}
              <div className="flex flex-col gap-3">
                  ...
                  <div key={type} className={`rounded-2xl border p-4 ${bg}`}>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => regenerateInviteCode(type)} className="p-2.5 rounded-xl bg-white/10 hover:bg-amber-500/20 hover:text-amber-400 transition-colors text-muted">
                          <RefreshCw size={16} />
                        </button>
                      </div>
                    </div>
                ...
                <form
                  onSubmit={async (e) => {
                    ...
                    if (fromEnv && amount > fromEnv.balance) {
                      setPtError(`「${fromEnv.name}」餘額不足（剩餘 ${currencySymbol}${fromEnv.balance}）`);
                      return;
                    }
                    setPtLoading(true);
                    try {
                      await transferMoney(ptFromId, ptToId, amount, ptNote || '家長互轉');
                      setPtAmount(''); setPtNote(''); setPtError(null);
                    } catch (err) {
                      setPtError((err as Error)?.message || '轉帳失敗');
                    } finally {
                      setPtLoading(false);
                    }
                  }}
                ...
                >
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        className={INPUT}
                        value={ptFromId}
                        onChange={e => { setPtFromId(e.target.value); setPtError(null); }}
                        required
                      >
                    ...
                    <div>
                      ...
                      <select
                        className={INPUT}
                        value={ptToId}
                        onChange={e => { setPtToId(e.target.value); setPtError(null); }}
                        required
                      >
                        <option value="">選擇...</option>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`${INPUT} ${ptError ? 'border-red-500/60' : ''}`}
                        value={ptAmount}
                        onChange={e => { setPtAmount(e.target.value); setPtError(null); }}
                        required
                      />
                    </div>
                    <div>
                  ...
                  {ptError && (
                    <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-xl px-3 py-2 text-sm text-red-400">
                      <span>⚠️</span><span>{ptError}</span>
                    </div>
                  )}
                  <button
                    type="submit"
                    disabled={ptLoading}
                    className={`${BTN} w-full bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 disabled:opacity-40`}
                  >
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 py-2 rounded-xl text-xs font-bold transition-colors"
                  >
                    📥 匯出 JSON
                  </button>
                  <button 
                    onClick={handleExportCSV}
                    className="flex items-center justify-center gap-2 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 py-2 rounded-xl text-xs font-bold transition-colors"
                  >
                    📊 匯出 CSV (交易)
                  </button>
                </div>
                ...
                <div className="relative">
                  ...
                  <button 
                    onClick={() => { if(confirm('⚠️ 警告：匯入資料將覆蓋目前的家庭設定。建議匯入前先手動備份目前資料。確定要繼續嗎？')) fileInputRef.current?.click(); }}
                ...
                {importStatus.type !== 'idle' && (
                  <div className={`text-[10px] px-3 py-2 rounded-lg flex items-center gap-2 ${
                    importStatus.type === 'loading' ? 'bg-blue-500/10 text-blue-300' :
                    importStatus.type === 'success' ? 'bg-green-500/10 text-green-300' :
                    'bg-red-500/10 text-red-300'
                  }`}>
                    {importStatus.type === 'loading' ? '⌛ 處理中...' : importStatus.type === 'success' ? '✅ ' : '❌ '}
      ...
      case 'history':
        return (
          <SortableSection key={id} id={id} emoji="📋" title="系統歷史紀錄" defaultOpen={false}>
            <div className="flex flex-col gap-4">
              <div className="flex gap-1 bg-black/30 p-1 rounded-2xl border border-white/5">
              ...
              {historyTab === 'quests' ? (
                <div className="flex flex-col gap-2">
                  {quests.filter(q => q.status !== 'open' && q.status !== 'pending_approval')
                    .slice(0, 30).map(q => {
                      const kid = users.find(u => u.id === q.ownerId);
                      const SM: Record<string,{label:string;color:string}> = {
                        completed:{label:'✅ 完成',color:'text-green-400'},
                        rejected:{label:'❌ 拒絕',color:'text-red-400'},
                        delayed:{label:'⏳ 延期',color:'text-orange-400'},
                      };
                      ...
                      return (
                        <div key={q.id} className="flex items-center gap-3 px-4 py-3 bg-white/5 border border-white/5 rounded-2xl">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-muted">{kid?.name}</div>
                          </div>
                          <span className={`text-xs font-bold flex-shrink-0 ${s.color}`}>{s.label}</span>
                        </div>
                      );
                  })}
                  {quests.filter(q => q.status!=='open'&&q.status!=='pending_approval').length===0 &&
                    <p className="text-center text-muted text-sm py-4">尚無歷史任務</p>}
                </div>
              ) : (
                <div className="flex flex-col gap-2">
                  {[...transactions].sort((a,b) => Number(b.timestamp) - Number(a.timestamp)).slice(0,30).map(t => {
                    ...
                    return (
                      <div key={t.id} className="flex items-center gap-3 px-4 py-3 bg-white/5 border border-white/5 rounded-2xl">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-bold truncate">{t.note||(t.amount>=0?'收入':'支出')}</div>
  ...
  return (
    <div className="flex flex-col gap-4 max-w-2xl mx-auto px-3 pb-24">
      {/* Top identity bar */}
      <div className="flex items-center gap-3 pt-1 pb-2">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 bg-amber-400/10 border border-amber-400/20 text-amber-400 text-xs font-bold px-3 py-1.5 rounded-full">
            <span>⚠️</span> {pendingCount > 0 ? `${pendingCount} 待審核` : '無待審'}
          </div>
        </div>
      </div>
...
const KidEnvelopeRow: React.FC<{
  ...
  protected?: boolean; // if true: cannot delete or hide
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden ${
      isProtected ? 'border-primary/20' : 'border-white/5'
    } ${env.isHidden ? 'opacity-50' : ''}`}>
      {isProtected && (
        <div className="text-[10px] text-primary/60 font-bold px-4 pt-2 flex items-center gap-1">
          <span>🔒</span> 受保護（不可刪除）
        </div>
      )}
        ...
        {editing ? (
          ...
          <>
            ...
            <div className="flex gap-1 flex-shrink-0">
              ...
              {!isProtected && (
                <button onClick={() => toggleHideEnvelope(env.id)} className={`${BTN_ROW} hover:bg-white/10 text-muted hover:text-white`}>
                  {env.isHidden ? <Eye size={14} /> : <EyeOff size={14} />}
                </button>
              )}
              {!isProtected ? (
...
const RoutineQuestPanel: React.FC<{
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (kid === 'all') {
      await addRoutineQuest(activeKids.map(k => k.id), title, +exp, +money, +gem, days, icon, undefined, boxId);
    } else {
      await addRoutineQuest(kid, title, +exp, +money, +gem, days, icon, undefined, boxId);
    }
  ...
  const startEdit = (rq: typeof routineQuests[0]) => {
    setEditId(rq.id);
    setEditTitle(rq.title);
    setEditIcon(rq.icon || '📅');
    setEditExp(String(rq.expReward));
    setEditMoney(String(rq.moneyReward));
    setEditGem(String(rq.gemReward));
    setEditDays([...rq.daysOfWeek]);
    setEditBoxRewardId(rq.boxRewardId || '');
  };
          ...
          {routineQuests.map(rq => {
              ...
              return (
                <div key={rq.id} className="flex flex-col gap-3 bg-primary/10 border border-primary/30 rounded-2xl px-4 py-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {DAYS_MAP.map(d => (
                        <button key={d.v} type="button" onClick={() => toggleEditDay(d.v)} className={`w-10 h-10 rounded-full font-black text-sm transition-all ${editDays.includes(d.v) ? 'bg-primary text-white' : 'bg-white/10 text-muted'}`}>
                          {d.l}
                        </button>
                      ))}
                    </div>
            ...
            return (
              <div key={rq.id} className="flex items-center justify-between gap-3 bg-black/30 border border-white/5 rounded-2xl px-4 py-3">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  <button onClick={() => startEdit(rq)} className="p-2.5 rounded-xl hover:bg-primary/20 text-primary/60 hover:text-primary transition-colors">
                    <Edit2 size={15} />
                  </button>
                  <button onClick={() => { if (confirm('刪除此固定任務？')) deleteRoutineQuest(rq.id); }} className="p-2.5 rounded-xl hover:bg-error/20 text-error/60 hover:text-error transition-colors">
                    <Trash2 size={15} />
                  </button>
                </div>
              </div>
